...
///指定用户的原始积分
@property(nonatomic) long long myOriginScore;

///指定用户的积分标签
@property(nonatomic,copy) NSString * myScoreTag;
...
+(void)reportScore:(long long)score leaderboardId:(NSString *)leaderboardId scoreTag:(NSString*)scoreTag success:(void (^)())success failure:(void(^)(NSError * error))failure;
